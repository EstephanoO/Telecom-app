...
async function LoadFormularios() {
  const response = await fetch('monorail.proxy.rlwy.net:24690/api/formularios');
  const data = await response.json();
  return data;
}

async function updateFormulario(id, data) {
  const response = await fetch(`monorail.proxy.rlwy.net:24690/api/formularios/${id}`, {
    method: 'PATCH',
    headers: {
...
function TablaPage() {
  ...
  const filteredData = data.filter((row) => {
    ...
    return nameMatch && trabajoMatch && fechaMatch && dateInRange;
  });
  ...
  const handleDownloadExcel = () => {
    const filteredDataForExport = filteredData.map((row) => ({
      ...
      Nombre: row.Nombre,
      Grupo: row.Grupo === 1 ? 'Si' : row.Grupo === 2 ? 'No' : '',
      NombresGrupo: row.NombresGrupo,
                ...
                >
                  {editingCell && editingCell.rowIndex === rowIndex && editingCell.cellIndex === 1 ? (
                    <>
                      ...
                      />
                      <IconButton
                        onClick={() => handleCellKeyDown(null, rowIndex, 2, row.ID)}
                      >
                      </IconButton>
                    </>
                  ) : (
                ...
                >
                  {editingCell && editingCell.rowIndex === rowIndex && editingCell.cellIndex === 3 ? (
                    <>
                      ...
                      />
                      <IconButton
                        onClick={() => handleCellKeyDown(null, rowIndex, 3, row.ID)}
                      >
                      </IconButton>
                    </>
                  ) : (
                    row.Grupo === 1 ? 'Si' : row.Grupo === 2 ? 'No' : ''
                  )}
                </TableCell>
                <TableCell
